(function () {
  ...
  function addAssistantMessage(answer, job) {
    ...
    if (job && job.id && answer.sources.length && answer.traceAvailable) {
      var auditButton = node("button", "detail-button", "Check citation support");
      auditButton.type = "button";
      var auditPanel = node("section", "citation-audit");
      auditPanel.hidden = true;
      auditPanel.setAttribute("aria-live", "polite");
      auditButton.addEventListener("click", function () {
        runCitationAudit(String(job.id), auditButton, auditPanel, answer, job);
      });
      actions.appendChild(auditButton);
      article.appendChild(auditPanel);
    }
  ...
  function showCitationAudit(audit, panel, answer, job) {
    panel.replaceChildren();
    panel.hidden = false;
    panel.appendChild(node("h3", "", "Citation support check"));
    var units = Array.isArray(audit.units) ? audit.units : [];
    var scored = units.filter(function (unit) { return unit.state === "scored"; });
    var flagged = scored.filter(function (unit) {
      return typeof unit.support_score === "number" && unit.support_score <= 0.5;
    });
    var literal = units.filter(function (unit) { return unit.state === "literal_quote_verified"; });
    var unchecked = units.length - scored.length - literal.length;
    var headline = flagged.length
      ? flagged.length + (flagged.length === 1 ? " passage may not follow" : " passages may not follow") + " from the attached citations."
      : (scored.length ? "No issue detected in the passages checked by the model." : "No generated passages were checked by the model.");
    panel.appendChild(node("p", flagged.length ? "audit-flag" : "", headline));
    panel.appendChild(node("p", "audit-note", scored.length + " model-checked · " + literal.length + " exact source quotations · " + unchecked + " unchecked"));
    panel.appendChild(node("p", "audit-note", "This local AI check can miss errors or raise false alarms. It was evaluated on English prose; code and tables may remain unchecked. An exact quotation confirms the wording only. The answer is preserved for you to assess with its sources."));
    units.forEach(function (unit) {
      var detail = node("details", "audit-unit");
      var label = "Not checked";
      if (unit.state === "scored") label = unit.support_score <= 0.5 ? "Possible source mismatch" : "No issue detected";
      if (unit.state === "literal_quote_verified") label = "Exact source quotation";
      detail.appendChild(node("summary", "", label));
      detail.appendChild(node("blockquote", "", String(unit.claim || unit.text || "")));
      if (unit.reason) detail.appendChild(node("p", "audit-note", String(unit.reason).replace(/_/g, " ")));
      if (typeof unit.support_score === "number") detail.appendChild(node("p", "audit-note", "Model support score: " + unit.support_score.toFixed(3) + " (not a probability that the answer is true)."));
      (Array.isArray(unit.citation_ids) ? unit.citation_ids : []).forEach(function (id) {
        var button = node("button", "detail-button", "Open " + String(id));
        button.type = "button";
        button.addEventListener("click", function () { showAnswerDetails(answer, job, "sources"); openSource(String(id)); });
        detail.appendChild(button);
      });
      if (unit.state === "scored" && unit.support_score <= 0.5) detail.open = true;
      panel.appendChild(detail);
    });
    if (typeof audit.elapsed_s === "number") panel.appendChild(node("p", "audit-note", "Additional check time: " + formatDuration(audit.elapsed_s)));
    if (audit.costs && typeof audit.costs.startup_elapsed_s === "number" && audit.costs.startup_elapsed_s > 0.1) {
      panel.appendChild(node("p", "audit-note", "Included local model loading: " + formatDuration(audit.costs.startup_elapsed_s)));
    }
    if (audit.message) panel.appendChild(node("p", "audit-note", String(audit.message)));
  }

  async function runCitationAudit(jobId, button, panel, answer, job) {
    if (button.disabled) return;
    button.disabled = true;
    panel.hidden = false;
    panel.replaceChildren(node("p", "audit-note", "Preparing the local citation check… The first check may need up to a minute to load its model, followed by up to 20 seconds of checking."));
    var sessionAtStart = state.session;
    var started = Date.now();
    try {
      var payload = await requestJson(API.job + encodeURIComponent(jobId) + "/audit", { method: "POST", body: {} });
      while (state.session === sessionAtStart && panel.isConnected) {
        var audit = payload.audit;
        if (!audit || typeof audit !== "object") throw new Error("The local citation check returned an invalid response.");
        if (audit.status === "complete") {
          showCitationAudit(audit, panel, answer, job);
          button.textContent = "Citation check complete";
          return;
        }
        if (audit.status === "failed") throw new Error(audit.message || (audit.error && audit.error.message) || "The local citation check could not finish.");
        var progressText = audit.phase === "loading_model"
          ? "Loading the local checking model… First use may take up to a minute. Your answer is ready above."
          : (audit.phase === "scoring" ? "Checking the attached sources locally… Up to 20 seconds of checking follows model loading." : "Preparing and verifying the saved answer and sources…");
        panel.replaceChildren(node("p", "audit-note", progressText));
        if (Date.now() - started > 105000) throw new Error("The citation check is still unavailable. The answer has been preserved.");
        await new Promise(function (resolve) { window.setTimeout(resolve, 600); });
        if (state.session !== sessionAtStart || !panel.isConnected) return;
        payload = await requestJson(API.job + encodeURIComponent(jobId) + "/audit");
      }
    } catch (error) {
      if (state.session !== sessionAtStart || !panel.isConnected) return;
      panel.replaceChildren(node("p", "audit-note", errorText(error)));
      button.textContent = "Citation check unavailable";
    }
  }
